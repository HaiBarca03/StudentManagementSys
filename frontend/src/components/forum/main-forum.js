...
const PostList = ({ posts, handleClick, page, totalPages, handlePageChange }) => {
              ...
              {featuredPosts.slice(1, 4).map((post) => (
                ...
                >
                  ...
                  <Grid item xs={post.thumbnail?.url ? 8 : 12}>
                    <CardContent sx={{ padding: 1.5, display: 'flex', flexDirection: 'column', gap: 1 }}>
                      {/* Thông tin tác giả */}
                      <Box display="flex" alignItems="center" gap={1}>
                        <Avatar
                          src={featuredPosts?.[0]?.userId?.avatar || ''}
                        ...
                        <Typography variant="body2" color="textSecondary">
                          {featuredPosts?.[0]?.userId?.name || 'Người dùng ẩn danh'} -{' '}
                          {featuredPosts?.[0]?.createdAt
                            ? new Date(featuredPosts[0].createdAt).toLocaleDateString('vi-VN', {
                                day: '2-digit',
                                month: '2-digit',
                                year: 'numeric',
                              })
...
const MainForum = () => {
  ...
  if (error) {
    return <Typography textAlign="center" color="error">{error}</Typography>;
  }
